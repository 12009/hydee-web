...
import com.hydee.hdsec.util.PushMessageUtils;
...
public class CompanyTrainTaskServiceImpl implements CompanyTrainTaskService {
    ...
    private static final String CONTENT = "{\"code\":\"50003\",\"msg\":\"您有一条新的通知,请注意查收!\"}";
     ...
     */
    @Override
    public int saveOrUpdateTask(CompanyTrainTask companyTrainTask,String connBusiGroupListJsonStr, Long classId, String thumbnails, String customer, CompanyUser user) throws Exception {
        int flag = 0;
        ...
        Long taskId = 0L;
        companyTrainTask.setResidueMoney(companyTrainTask.getMoneyReward());//剩余钱于总额一致
        if (null == companyTrainTask.getTaskId()) {
            ...
            companyAccount.setBalance(trainTask.getMoneyReward());
        ...
        if(user.getIsXmCustomer()){//小蜜连锁账号插入门店编号
            JSONArray busiGroups = JSONArray.fromObject(connBusiGroupListJsonStr);
            for (int j = 0; j < busiGroups.size(); j++) {
                CompanyTrainTaskCustomer trainTaskCustomer = new CompanyTrainTaskCustomer();
                JSONObject jsonObject = (JSONObject) busiGroups.get(j);
                trainTaskCustomer.setCustomerId(user.getXmCustomerId());
                trainTaskCustomer.setBusno((String) jsonObject.get("busno"));
                trainTaskCustomer.setCustomerName((String) jsonObject.get("name"));
                trainTaskCustomer.setTaskId(taskId);
                trainTaskCustomer.setIsStores(1);
            ...
            for (int j = 0; j < customers.size(); j++) {
                ...
                trainTaskCustomer.setTaskId(taskId);
                trainTaskCustomer.setIsStores(0);
                trainTaskCustomerList.add(trainTaskCustomer);
            }
        }
    ...
    public int updateTaskMoney(CompanyTrainTask trainTask,int type,double money) {
        double _residueMoney = trainTask.getResidueMoney();
        double _money = 0;
        if(1 == type){//新增余额
            _money = _residueMoney+money;
        }else if(2 == type){//减少
            _money = _residueMoney-money;
        }else{
            _money = _residueMoney;
        }
        trainTask.setStatus(2);
        trainTask.setResidueMoney(_money);
        return companyTrainTaskDao.updateTaskById(trainTask);
    }
    ...
    public int openMoney(CompanyTrainTaskUser taskUser, CompanyTrainTask trainTask, CompanyTrainTaskLockMoney taskLockMoneys, String trainErrorOptions) throws Exception {
        ...
        double amount = trainMoney + redMoney;//总共需要支付
        double customerBalance = trainFund;
        double deductMoney = 0;//需要扣除的总余额
        double _money = 0;
        double deductServeCharge = serveCharge;//需要扣除的平台服务费
        double deductTrainFund = trainFund;//需要扣除的连锁服务费
        if(redMoney == 0){

        }else if (lockMoney <= redMoney) {//如果锁定的余额小于红包
            redMoney = lockMoney;
        }else if(lockMoney > redMoney){//如果锁定余额大于红包，
            _money = lockMoney - redMoney;//扣除红包所剩余额
            if(_money < customerBalance){//扣除红包后的钱小于连锁的钱
                deductTrainFund = _money - customerBalance;
                deductServeCharge = 0;
            }else if(_money - customerBalance < deductServeCharge){
                deductServeCharge = _money - customerBalance;
            }
        }
        deductMoney = redMoney + deductTrainFund + deductServeCharge;
        taskUser.setRedMoney(redMoney);
        taskUser.setCorrect(correct);
        companyTrainTaskDao.insertTaskUser(taskUser);

        updateTaskMoney(trainTask,1,lockMoney);//返回任务锁定的余额

        updateTaskMoney(trainTask,2,deductMoney);//扣除红包和服务费的钱
    ...
    public int issueTask(CompanyTrainTask companyTrainTask, CompanyUser user) throws Exception {
        ...
        if(!user.getIsXmCustomer()){
            ...
            for (CompanyTrainTaskCustomer taskCustomer : taskCustomerList) {
                List<MobileUser> mobileUsers = mobileUserService.queryMobileUserByCustomerId(taskCustomer.getCustomerId());
                Message message = new Message();
                message.setCustomerId(taskCustomer.getCustomerId());
                message.setType("0");
                message.setContent("小蜜消息：【厂家培训】上新了，邀请您赶快参与，考试完可以领取红包哟！");
                message.setCreaterId("0");
                message.setAppPageType(1);
                message.setSourceId(taskCustomer.getTaskId().toString());
                List<String> cids = new ArrayList<String>();
                List<MessageUser> messageUsers = new ArrayList<MessageUser>();
                List<CID> cidList = new ArrayList<CID>();
                String _id = UUID.randomUUID().toString().replace("-", "");
                message.setId(_id);
                for (MobileUser mobileUser : mobileUsers) {
                    String _cid = null == mobileUser.getCid() ? "" : mobileUser.getCid();
                    MessageUser messageUser = new MessageUser();
                    messageUser.setCustomerId(taskCustomer.getCustomerId());
                    messageUser.setUserId(mobileUser.getUserId());
                    messageUser.setUserName(mobileUser.getUserName());
                    messageUser.setUserPhone(mobileUser.getMobileNo());
                    messageUser.setHasRead("0");
                    messageUser.setMessageId(_id);
                ...
                if(messageUsers.size() > 0){
                    ...
                    if(flag <=0 ){
                        throw new ServiceException(ErrorCodes.MESSAGE_INSERT_DEFEATED);
                    }
                    messageDao.insertMessageUser(messageUsers);
                    PushMessageUtils.pushToListAsyn(cidList,CONTENT);
                }
            }
        }
        return 0;
    ...
    @Override
    public int getRedByCidAndUserId(Map<String, Object> map) throws Exception {
        return companyTrainTaskDao.getRedByCidAndUserId(map);
    }

    @Override
    public int sendMessageTest(List<String> customerIds) throws Exception {
        for (String taskCustomer : customerIds) {
            List<MobileUser> mobileUsers = mobileUserService.queryMobileUserByCustomerId(taskCustomer);
            Message message = new Message();
            message.setCustomerId(taskCustomer);
            message.setType("0");
            message.setContent("小蜜消息：【厂家培训】上新了，邀请您赶快参与，考试完可以领取红包哟！");
            message.setCreaterId("0");
            message.setAppPageType(1);
            message.setSourceId("166");
            List<String> cids = new ArrayList<String>();
            List<MessageUser> messageUsers = new ArrayList<MessageUser>();
            List<CID> cidList = new ArrayList<CID>();
            String _id = UUID.randomUUID().toString().replace("-", "");
            ...
            for (MobileUser mobileUser : mobileUsers) {
                String _cid = null == mobileUser.getCid() ? "" : mobileUser.getCid();
                MessageUser messageUser = new MessageUser();
                messageUser.setCustomerId(taskCustomer);
                messageUser.setUserId(mobileUser.getUserId());
                messageUser.setUserName(mobileUser.getUserName());
                messageUser.setUserPhone(mobileUser.getMobileNo());
                messageUser.setHasRead("0");
    ...
    public int extract(Long taskId, CompanyUser _user) {
        ...
        //任务没有结束
        if (DateUtil.compareDate(task.getEndTime(),new Date())) return 0;
