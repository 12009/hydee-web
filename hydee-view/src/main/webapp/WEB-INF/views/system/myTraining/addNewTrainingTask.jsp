        ...
        <div class="marginRight10">
            <div class="task-main">
                ...
                <ul class="ul-info heightAuto vertical-align-top">
                    <li class="width80 textAlignR marginRight20">
                ...
                <c:if test="${!sessionUser.isXmCustomer}">
                    <ul class="ul-info heightAuto vertical-align-top">
                        <li class="width80 textAlignR marginRight20"></li>
                        <li>
                            <ul class="ul-multipleShop-Upload max-height" style="overflow: auto" id="addShopMain">
                ...
                <c:if test="${sessionUser.isXmCustomer}">
                    <ul  style="height: 272px; border: 1px #C1CDCD solid; width: 700px; padding-right: 10px;" class="ul-info vertical-align-top" id="erpBusiDialog">
                        <li class="width80 textAlignR marginRight20"></li>
                        <li>
                            <input type="hidden" name="connBusiGroupListJsonStr" value="" class="required" />
                        </li>
                        <div class="clear"></div>
                    </ul>
                </c:if>
                ...
                <ul class="ul-info table-height30">
                    ...
                    <li class="width170">
                        <label class="input">
                            ...
                            <input type="hidden" value="${companyTrainTask.moneyReward}" id="reward">
...
<div class="shadowWhite displayNone trainingDetail"><!--查看详情-->
    <div class="contentWrapper">
        <div class="content">
            ...
            <div class="div-ui-divide marginRight10"><span class="span-ui-dot"></span><span>配套练习</span><span id='baseType'></span></div>
...
<div class='chooseProduce'>
    <div class='cProduceHeader'>
        ...
        <a id="checkBoxAll" class="cProduceHeaderLeft marginLeft20" style="color: #316ac5">[全选]</a>
...
<script>
    ...
    var _pageSize = 30,_nowPage = 0,_count = 0,_totalPage,
        _pageCount;
    var _companyMap = new HashMap();
    var chooseId = [];
    console.log(${sessionUser.isXmCustomer});

    $(function(){
        if(${!sessionUser.isXmCustomer}){
            $.ajax({
                url 	 : "${mvcPath}/company/listCompany",
                type 	 : "post",
                dataType : "JSON",
...
//                    if(i == 10) break;
        ...
        $("#awardHigh").blur(function(){
            ...
            if("" != _awardHigh && "" != _moneyReward){
                ...
                if(1 == _brokerage){
                    _trainFund = ${sessionUser.isXmCustomer} ? '0' : $("#trainFund").val();
                    ...
                    var _fund,_charge;
                    _trainFund = ${sessionUser.isXmCustomer} ? '0' : $("#trainFundPercent").val();
...
//            } else {
//                $(".cProduceSelectedShow ul li").parent().empty();
//                $(".cProduceMain li").each(function(){
...
//        $(".cProduceSelectedShow ul").empty();
    ...
    function saveTask(){
        ...
        if(_params != null){
            ...
            if(${sessionUser.isXmCustomer}){
                var _connBusiGroupListJsonStr = $("input[name='connBusiGroupListJsonStr']").val();
                if($.parseJSON(_connBusiGroupListJsonStr).length <= 0 || $.parseJSON(_connBusiGroupListJsonStr) == null){
                    $.hd.tips($("#addShop"),"请指定门店");
                    return ;
                }
                _params.connBusiGroupListJsonStr = _connBusiGroupListJsonStr;
                console.log(_params);
            }else{
...
//                }
            ...
            if(Number($('#availableBalance').val())+Number($('#reward').val()) < Number($('#moneyReward').val()) ){
    ...
    if(${sessionUser.isXmCustomer}){
        // 初始化门店列表
        var token_erpBusiList = "erpBusiList_${sessionUser.customerId}";
        var busiList = Cache.load(token_erpBusiList);
        var $connBusiGroupListJson = $("#erpBusiDialog").find("input[name=connBusiGroupListJsonStr]");
        var selConnBusiGroupList = ${taskCustomerList == null ? 'null' : taskCustomerList};
        <%--//${group == null ? 'null' : group.connBusiGroupListJsonStr}--%>
        if(busiList == null || busiList.length == 0){
            // 请求ERP门店列表数据
            console.log(${params});
            $.ajax({
                type : "POST",
                url : _companyUrl+"/api/requestSql/${hType}/contactGetOrgBusi",
                data : $.extend(${params},{zlln_key:getVerifyCode()}),
                success : function(rsp) {
                    rsp = typeof rsp == "string" ? JSON.parse(rsp) : rsp;
                    console.log(rsp);
                    if(rsp == null || rsp.length == null){
//                    alertMsg.error("门店数据获取失败,请在APP重新登录后重试!", {
//                        okCall :function(){
//                            $.pdialog.closeCurrent();
//                        }
//                    });
                        $.hd.alertMsg.error("门店数据获取失败!");
                    }else{
                        busiList = [];
                        $.each(rsp, function(i, item){
                            busiList.push({
                                busiNo: item[0],
                                busiName: item[2],
                                busiType: item[8]	// 0:总部,1:直营,2:加盟
                            });
                        });
                        // 缓存ERP用户数据
                        Cache.save(token_erpBusiList, busiList);
                        // 初始化ERP人员列表
                        initErpBusiList(busiList);
                    }
                },
                error : function(XMLHttpRequest, textStatus, errorThrown) {
                    console.log(textStatus);
        ...
        function initErpBusiList(data){
            $("#erpBusiDialog").erpBusies("${companyName}", data, selConnBusiGroupList, function(rsp){
                var _tempBusiListData = [];
                // 转义实体类属性名,用于后台老接口实例化JSON对象
                $.each(rsp, function(i, busi){
                    _tempBusiListData.push({
                        busno: busi.busiNo,
                        name: busi.busiName
                    });
                })
                $connBusiGroupListJson.val( JSON.stringify(_tempBusiListData) );
            });
        }
    }
