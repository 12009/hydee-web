...
<div class="right-main">
    ...
    <c:forEach items="${reList}" var="item">
        <ul class="ul-table-tBody tableWithoutLine100">
            ...
            <c:if test = "${sessionUser.isSysAdmin}">
                <li class="width80" title="${item.customerName}">${item.customerName}</li>
            </c:if>
                ...
                <c:choose>
                    <c:when test = "${sessionUser.isSysAdmin}">
                        <c:if test = "${item.status != 4 || item.status != 5}">
                            ...
                            <a class="operation preview">预览</a>
                        </c:if>
...
<div class="sjyl">
    <div class="sj">
        <div class="sj_content">
            <div class="sj_top" id="classTitle"></div>
            <div class="sj_center" id="content" style="width: auto"></div>
            <div class="sj_bottom">学习完毕，答题抢赏金</div>
        </div>
    </div>
    <div class="sjClose"></div>
</div>
</div>
        ...
        $(".preview").bind("click",function(){
            ...
            $.ajax({
                ...
                success  : function(data, textStatus, XMLHttpResponse) {
                    ...
                    $("#classTitle").html('');
                    $("#content").html('');
                    $("#classTitle").html(_companyTrainClass.title);
                    var _content = _companyTrainClass.content;
                    if(1 == _companyTrainClass.contentType){
                        $.hd.pdfFile(_content,$('#content'));
                    }else{
                        $("#content").html(_companyTrainClass.content);
                    }
                    $(".sjyl,.bgShow").fadeIn();
