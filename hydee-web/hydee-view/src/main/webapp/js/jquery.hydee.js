...
(function($){
	...
	hd:{
		...
		alertMsg : {
			version	: "v1.0",
			window	: null,
			init	: function(title,msg,success){
				var that = this;
				// 初始化HTML
				if(!$(".bgShow").length){
				...
				if(!$("#hd_alertMsg").length){
					var html  = '<div id="hd_alertMsg" class="shadowWhite displayNone fabu_1">';
						...
						html += '<li class="detailMsg" style="margin-top:80px; margin-left:70px; font-size:16px;">'+msg+'</li>';
						html += '</ul></div>';
			...
			open	: function(showMask,timeout){
			...
			// 普通窗口弹出
			info	: function(msg,title,showMask){
				this.init(title || "信息",msg);
				this.open(showMask || false, 0);
			},
			// 错误提示窗口
			error	: function(msg,title,showMask){
				this.init(title || "错误",msg);
				this.open(showMask || false, 0);
			},
			// 确认提示窗口
			confirm	: function(title,msg,sucess,showMask){
				this.init(title,msg,sucess);
				this.open(showMask || true, 0);
			}
		},
		/*
		 * 提示气泡简易封装
			...
			$.ajax({
				...
				success  : function(data, textStatus, XMLHttpResponse) {
					...
					for(var i=1;i<=_num;i++){
						_html += '<li><img src="'+ _content +'?odconv/jpg/page/'+ i +'"></li>'
					}
					_html += '</ul>';
					_div.append(_html);
